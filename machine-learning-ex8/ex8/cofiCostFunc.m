...
prediction = X * Theta';            % Predict movie ratings
error = (prediction - Y) .^2;       % Determine error for each touple movie/user
J = 1/2 * sum(sum(error .* R));     % Sum over errors IF movie was rated

J = J + lambda*sum(sum(Theta.^2))/2;% regularized term of Theta.
J = J + lambda*sum(sum(X.^2))/2;    % regularized term of X.


% Calculate gradients of X and Theta
diff = (X * Theta' - Y);            % Determine error for each touple movie/user
